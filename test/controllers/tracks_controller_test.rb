...
class TracksControllerTest < ActionDispatch::IntegrationTest
  ...
  test "the trace is reachable without authentication" do
    get track_path(@vessel.track_slug)

    assert_response :success
    assert_select "h1", @vessel.name
  end

  test "an unknown slug is a 404, not a redirect to the login page" do
    get track_path("notatrackingslugatall")

    assert_response :not_found
  end

  test "a slug too short to be one does not even match the route" do
    get "/suivi/nope"
  ...
  test "the English path the route used to answer on is gone" do
    get "/track/#{@vessel.track_slug}"

    assert_response :not_found
  end

  # The route constraint and Vessel::Tracking::SLUG_FORMAT are written out in
  # two files that cannot see each other. Should they ever drift, a slug the
  # skipper was allowed to save would 404 for the family ashore.
  test "the route accepts exactly the slugs the model allows" do
    %W[
  ...
  test "the trace is downloadable as KML for Google Earth" do
    ...
    get track_path(@vessel.track_slug, format: :kml)
  ...
  test "an unknown slug is a 404 for the KML too" do
    get track_path("notatrackingslugatall", format: :kml)
  ...
  test "the trace page offers the KML download" do
    ...
    assert_select "a[href=?]", track_path(@vessel.track_slug, format: :kml)
  end
  ...
  test "the vessel page links to the shareable trace" do
    ...
    assert_response :success
    assert_select "a[href=?]", track_path(@vessel.track_slug)
  end

  private
    def model_accepts?(slug)
      vessel = Vessel.new(track_slug: slug)
      vessel.valid?

      !vessel.errors.include?(:track_slug)
    end

    def route_matches?(slug)
      Rails.application.routes.recognize_path("/suivi/#{slug}")
      true
