module Vessel::Commanding
  ...
  def execute_drop(args, results)
    if args&.upcase == "LAST"
      last_bundle = bundles.sent.recent.first
      if last_bundle
  ...
  def execute_status(results)
    pending_count = mail_accounts.joins(:collected_messages).where(collected_messages: { status: "pending" }).count
    results << {
      command: "STATUS",
      status: :ok,
  ...
  def resolve_outbound_account(recipient)
    ...
    previous ? previous.mail_account : (mail_accounts.find_by(is_default: true) || mail_accounts.first)
  end
